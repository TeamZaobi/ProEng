...
`$product-engineer` 做需求分析、PRD、架构说明或方案判断时，默认按这条因果链回溯：

`人性的贪婪和恐惧 -> 痛点 -> 需求 -> 架构 -> 方案`

这里的“贪婪和恐惧”用于定位痛点真源，不用于道德评价用户。
所谓贪婪，是同类竞争中展示比较优势；所谓恐惧，是环境压力下增强生存能力。
痛点通常来自这两类驱动力在具体场景中的受阻、放大或失衡。

默认要求：

1. 痛点必须能回指到真实的人性驱动力：比较优势展示或生存能力增强，而不只是“用户觉得不方便”。
2. 需求必须能解除或缓解明确痛点，而不只是把一个想法写成 feature。
3. 架构必须由需求推出，而不从技术偏好、owner 分工、文件位置或现成工具反推需求。
4. 方案必须回指架构和需求，并说明它怎样降低使用者的混乱、摩擦、风险或认知负担。
5. 如果缺少上游因果链，只能输出假设或追问；不要直接给看似完整的方案。
